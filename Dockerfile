...
# TIMEZONE in php
RUN sed -i "s@^;date.timezone =.*@date.timezone = $TIMEZONE@" /etc/php/7.0/*/php.ini

# DEV conf for php
RUN sed -i "s@^error_reporting =.*@error_reporting = E_ALL@" /etc/php/7.0/*/php.ini
RUN sed -i "s@^display_errors =.*@display_errors = On@" /etc/php/7.0/*/php.ini
RUN sed -i "s@^display_startup_errors =.*@display_startup_errors = On@" /etc/php/7.0/*/php.ini
